...
import {applyTheme as applyBaseTheme} from 'themes/my-theme/my-theme.generated.js';
...
window.Vaadin.Flow['_vaadintheme_my-extended-theme_globalCss'] = window.Vaadin.Flow['_vaadintheme_my-extended-theme_globalCss'] || [];
export const applyTheme = (target) => {
  applyBaseTheme(target);

  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_my-extended-theme_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_my-extended-theme_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_my-extended-theme_globalCss'].push(target);
  }
  if (!document['_vaadintheme_my-extended-theme_componentCss']) {
    
    document['_vaadintheme_my-extended-theme_componentCss'] = true;
  }
