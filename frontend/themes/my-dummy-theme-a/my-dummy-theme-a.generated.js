...
window.Vaadin.Flow['_vaadintheme_my-dummy-theme-a_globalCss'] = window.Vaadin.Flow['_vaadintheme_my-dummy-theme-a_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_my-dummy-theme-a_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_my-dummy-theme-a_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_my-dummy-theme-a_globalCss'].push(target);
  }
  if (!document['_vaadintheme_my-dummy-theme-a_componentCss']) {
    registerStyles(
      'vaadin-button',
      css`
    ...
    document['_vaadintheme_my-dummy-theme-a_componentCss'] = true;
  }
